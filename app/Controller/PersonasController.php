...
class PersonasController extends AppController{
	...
	public function ajax_list_personas(){
        ...
        foreach ($arr_obj_persona as $obj_persona) {
           ...
           $html = '<div id ="data_rol" class="div-row-persona" persona_id="'.$obj_persona->getAttr('id').'" persona_nombre = "'.$obj_persona->getAttr('nombre').' '.$obj_persona->getAttr('apellido').'"><a><i class="icon-pencil edit-persona-trigger"></i> </a> 
	...
	public function add_edit_persona($persona_id=null){
		...
		$this->set(compact('list_all_departamentos','list_all_tipo_personas','list_all_estados_civiles', 'list_all_roles'));
		...
		if($this->request->is('post')  || $this->request->is('put')){
			if(isset($persona_id) && intval($persona_id) > 0){
				...
				$role_id = $this->request->data['Persona']['rol_id'];
				$this->request->data['Persona']['estado'] = 1;
